"""Load text features from the ``features/`` directory.
...
This module exposes only the text (CINO-large-v2 ``last_hidden_state``) branch.
...
Example:
    ...
    text = load_text("2079_000023_angry", to_float=True)  # (T_t, 1024) float32
    text, mask = load_text_batch(["2079_000023_angry", "1023_000038_neutral"])
...
__all__ = ["load_text", "load_text_batch"]
...
def load_text(name: str, to_float: bool = False) -> torch.Tensor:
    """Load the text feature of a single sample.
    ...
    Returns:
        Text feature tensor of shape ``(T_t, 1024)``, where ``T_t`` is the
    ...
    text = data.get("text")
    _check_feature(text, stem, "text")
    return text.float() if to_float else text


def load_text_batch(names: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
    """Load and pad a batch of text features (mirrors ``collate_fn_precomputed``).
    ...
    Returns:
        A tuple ``(text, mask)``:
            - ``text``: ``(B, T_max, 1024)`` float32, padded to the longest
              sequence in the batch.
            - ``mask``: ``(B, T_max)`` int64, 1 for valid tokens, 0 for padding.
    """
    return _pad([load_text(name) for name in names])


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Load text features from features/.")
    parser.add_argument("--names", nargs="+", required=True, help="One or more sample names.")
    return parser


def main() -> None:
    """Command-line entry point: load and summarize text features."""
    args = _build_parser().parse_args()
    text, mask = load_text_batch(args.names)
    print(f"text: {tuple(text.shape)} dtype={text.dtype}")
    print(f"mask: {tuple(mask.shape)} dtype={mask.dtype}")
    for name, row_mask in zip(args.names, mask):
        print(f"  {name}: valid tokens={int(row_mask.sum())}")
